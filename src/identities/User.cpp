...
bool User::isSuperAdmin() const {
    ...
    return roles.admin->permissions.identities.admins.canDelete()
        && roles.admin->permissions.vaults.admin.permissions.keys.encryptionKey.any();
}
...
std::string to_string(const std::shared_ptr<User>& user) {
    ...
    out += "Password Changed At: " + (user->meta.password_changed_at ? timestampToString(*user->meta.password_changed_at) : "Unknown") + "\n";
...

    {
        ...
        out += user->roles.vaults.empty() ? "  None\n" : user->roles.admin->permissions.vaults.toString(2);
