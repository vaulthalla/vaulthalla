...
void vh::db::DBConnection::initPreparedVaultRoleAssignments() const {
    ...
    // LEGACY CODE - may want to move toward group ownership rather than owning group vault roles for user

    conn_->prepare(
        "get_user_and_group_assigned_vault_roles",
        R"SQL(
        (
            SELECT
                vra.id                           AS assignment_id,
        ...
        ORDER BY vault_id, subject_type, subject_id
    )SQL");
}
