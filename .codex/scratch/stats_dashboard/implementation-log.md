...
- Collapsed the dashboard home into a compact command strip plus the persisted configurable card grid.
- Moved customization controls out of a dedicated Home Layout panel and into the command strip.
- Removed the section summary row so users reach their configured cards immediately.
- Reworked attention rendering into a compact conditional strip with top-three visible issues and duplicate display suppression.
- Reworked summary cards for denser title/severity rows, shorter summaries, stronger numeric tiles, fewer issue rows, and lower minimum heights.
- Added frontend metric display preferences by card ID and size/variant.
- Demoted low-value metrics, especially `system.trends` `series` and `points`; zero cache evictions and zero unauthenticated sessions are suppressed from compact tiles.
- Added lightweight CSS meters for hit-rate/error-rate/cache-hit/pressure metrics already present in `stats.dashboard.overview`.
- Updated dashboard grid breakpoints to `base:1`, `md:6`, and `lg:12` columns.
- Removed the dashboard route centering wrapper and expanded dashboard home max width to `92rem`.
...
- Server-backed preferences, localStorage fallback/cache, presets, drag/drop, Up/Down fallback, add/remove, size/variant controls, reset, save, and fixed drilldown pages.
- Backend-provided severity/warnings/errors remain authoritative.

Layout/sidebar decision:

- A route-specific compact admin sidebar was deferred. The current sidebar is owned by the parent admin layout and rendered outside the dashboard route segment, so converting it cleanly would require a shared admin shell seam rather than a dashboard-home patch.
## Phase 16 - Dashboard Insight Cards and Visual Variants

- Commit: `82795125`.
- Push target: `origin/stats-dashboards`.
- Push result: succeeded, with GitHub remote moved warning.
- Added `web/src/components/dashboard/dashboardMetricCuration.ts` for home-card metric preference and low-value metric filtering.
- Added the `visual` variant to the finite dashboard card variant model.
- Updated `DashboardOverview` summary cards with denser metric tiles, variant-specific metric/issue counts, CSS ratio meters, and stacked bars for operation, connection, and storage mix cards.
- Updated `dashboardCardCatalog.ts` defaults and presets so the out-of-box dashboard home emphasizes operations, storage, FUSE, connection, DB, retention, and trend status without bloated full-width cards.
- Updated `DashboardOverview.cpp` summary builders with better home-card metric keys:
  - `system.threadpools`: `pressured`, `saturated`
  - `system.connections`: `share`
  - `system.storage`: `local`
  - `system.operations`: `failed_24h`
  - `system.trends`: `latest_sample_age`, `coverage`
- Preserved backend-owned severity/warning/error logic and existing drilldown cards.
...
- Push target: `origin/stats-dashboards`.
- Push result: succeeded, with GitHub remote moved warning.
- Removed shell admin UID from SystemHealth overall status computation.
- Updated `vh status` to mark unbound shell admin UID as a setup advisory.
- Updated dashboard overview `system.health` builder so shell admin UID setup is emitted as an info metric (`shell_admin_uid`) rather than a warning issue.
- Removed `system.health` from the frontend customizable card catalog, default dashboard layout, presets, and add-card menu.
- Kept `/dashboard` requesting `system.health` in `stats.dashboard.overview` so the command bar owns the permanent System Health surface.
- Added command-bar health metric pills for services, protocols, and dependencies.
- Added an info advisory pill in the command bar for unconfigured CLI shell admin UID.
- Updated the runtime drilldown SystemHealth card so shell UID setup is cyan/info rather than degraded/amber.
...
- Push target: `origin/stats-dashboards`.
- Push result: succeeded, with GitHub remote moved warning.
- Added `web/src/components/dashboard/DashboardRouteToolbar.tsx` for dashboard-specific route navigation across Overview, Runtime, Filesystem, Storage, Operations, and Trends.
- Updated `web/src/app/(app)/(admin)/dashboard/layout.tsx` to provide the dashboard toolbar and wider dashboard route canvas.
- Removed dashboard drilldown child links from `web/src/config/nav/admin.ts`; the top-level Dashboard nav item still shows overview severity.
- Replaced the old card add dropdown/add button pair in `DashboardOverview.tsx` with a visual picker panel:
  - size filter
  - variant selector
  - preview cards
  - one-click add
  - hidden already-visible cards
- Added normal-mode title-row drag handles using the existing dashboard preference save flow; customize-mode drag and Up/Down controls remain.
- Reworked summary cards to be bounded, denser, and variant-aware:
  - tighter title rows
  - smaller metric tile padding
  - capped summaries and issue lines
  - grid row spans for configured sizes
  - overflow protection for bounded cards
- Added `1x2` and `3x1` size support to the finite dashboard size model.
- Updated `dashboardMetricCuration.ts` with expanded high-signal metric preferences and visual-metric omissions so visual cards do not duplicate the metric represented by the visual.
- Expanded visual treatments:
  - operations pending/active/stalled/failed stack
  - thread pool pressure meter/worker pressure fallback
  - connection session mix stack
  - storage local/S3/inactive mix stack
  - retention overdue/expired/sync/audit stack
  - trends coverage/status strip
  - FUSE/cache/DB ratio meters through existing metric data
- Updated `dashboardCardCatalog.ts` defaults and presets:
  - Thread Pools are included by default.
  - Trends is not a default card unless selected through a preset/picker.
  - Presets avoid the worst awkward holes by using cleaner full-row and paired-card groups.
- Updated `core/src/stats/model/DashboardOverview.cpp` summary builders with more useful overview metric keys for the home dashboard.
- Preserved backend-owned severity/warning/error semantics and all fixed drilldown pages.
- Deferred route-specific collapsed admin sidebar because the current sidebar lives in the parent admin shell; a shared shell seam is the correct future implementation point.
...
- Push target: `origin/stats-dashboards`.
- Push result: succeeded, with GitHub remote moved warning.
- Updated `web/src/components/nav/admin/AdminSidebar.tsx`:
  - converted to a route-aware client component
  - detects `/dashboard` and `/dashboard/*`
  - uses `w-20`, compact `NavList`, compact toggle/logout controls, compact logo treatment on dashboard routes
  - preserves `w-80` full sidebar for non-dashboard admin routes
- Updated `web/src/components/dashboard/DashboardRouteToolbar.tsx`:
  - route tabs now sit inside a compact bordered toolbar strip
  - tabs have uniform height/spacing and retained live severity badges
  - horizontal overflow remains available on narrow widths
- Updated `web/src/components/dashboard/DashboardOverview.tsx`:
  - renamed/clarified home-card helpers as `DashboardHomeCard`, `DashboardMetricTile`, and `DashboardCardVisual`
  - cards are clickable in normal mode with keyboard link behavior
  - removed the `View details >` footer
  - made the card title row the normal-mode drag handle, eliminating hidden drag placeholder spacing
  - added a short drag suppression window so reorder gestures do not immediately navigate
  - tightened metric capacity by size/variant, especially for visual cards
  - retained customize-mode controls and server preference persistence
- Updated `web/src/components/dashboard/dashboardCardCatalog.ts`:
  - default `system.operations` and `system.storage` cards use `2x1` instead of `2x2`
  - cockpit preset also uses `2x1` operation/storage cards for cleaner row balance
- No backend changes were needed.
- No sparkline was added because no real compact trend point arrays are currently present in `stats.dashboard.overview`; this remains a backend payload follow-up.

## Dashboard Sidebar Build Fix

- Commit: pending.
- Push target: `origin/stats-dashboards`.
- Fixed Next.js prerender failure from passing function-valued nav icon components into a Client Component.
- Added `web/src/components/nav/admin/AdminSidebarMode.client.tsx`.
- Updated `web/src/components/nav/admin/AdminSidebar.tsx` so:
  - sidebar/nav rendering remains server-side
  - `AdminSidebarMode` is the only client boundary
  - route-aware compact/full selection still works for dashboard routes
  - `adminNav` icon functions no longer cross a server-to-client prop boundary
...
- Push target: `origin/stats-dashboards`.
- Push result: succeeded, with GitHub remote moved warning.
- Updated `web/src/components/nav/admin/AdminSidebarMode.client.tsx`:
  - adds user-toggleable compact/full sidebar mode
  - stores local preference under `vaulthalla.admin.sidebar.compact.v1`
  - defaults dashboard routes to compact when no explicit user preference exists
- Updated `web/src/components/nav/admin/AdminSidebar.tsx`:
  - keeps nav rendering server-side while mounting a compact/full toggle button inside both server-rendered variants
- Updated `web/src/config/nav/admin.ts`:
  - Dashboard item uses `gauge-high` instead of a vault icon
- Updated `web/src/components/nav/util.tsx`:
  - compact nav buttons center icons inside fixed icon-first controls
- Updated `web/src/components/dashboard/DashboardRouteToolbar.tsx`:
  - top dashboard nav is more uniform and uses the folder-tree icon for Filesystem
- Updated dashboard layout model/catalog:
  - added `instanceId` / `instance_id`
  - added `dashboardLayoutInstanceId()`
  - normalized older preferences without instance IDs
  - allowed same card family with different variants
  - prevented exact same card/variant duplicates
- Updated `DashboardOverview.tsx`:
  - card controls, drag/drop, save/reset, and picker now operate by instance ID
  - overview responses still request backend summaries by card id/size/variant in layout order
  - picker marks same card/variant as already added while preserving duplicate-family variant choices
  - size-to-metric capacity table controls visible metric counts
...
- Push target: `origin/stats-dashboards`.
- Backend:
  - Added `DashboardGraphPoint` and `DashboardGraphSeries` to `DashboardOverview`.
  - Added `DashboardCardSummary.series`.
  - Serializes graph series as compact card summary payload data.
  - Attaches trend series only for graph cards or the Trends card.
  - Caps card graph points to the latest 64 samples per series.
  - Reuses existing `stats_snapshot` runtime samples.
  - Extends `stats_snapshot.system_trends` with `threadpool_pool_pressure:<pool>` from `payload->pools`.
- Frontend:
  - Added `DashboardGraphPoint` / `DashboardGraphSeries` parser classes.
  - Added `graph` to finite card variants.
  - Added `DashboardSparkline` and graph-first card rendering.
  - Added graph metric omission rules so values represented by a graph are not duplicated as primary metric tiles.
  - Added graph-capable catalog support for Thread Pools, FUSE, FS Cache, HTTP Cache, DB, and Trends.
