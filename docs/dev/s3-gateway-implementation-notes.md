...
- Budget denials return S3 XML `AccessDenied` with HTTP 403.
...
- Web console route `/s3-gateway` exists with service, credential, scope, bucket, budget, ledger, and client setup sections.
...
- DB-backed route coverage now includes remote-backed PUT, GET, DELETE, copy-object, and multipart initiate/upload-part/complete budget preflight/commit ledger rows, budget denial returning S3 XML `AccessDenied` before the fake upstream PUT is called, release of an upload-part reservation after a pre-upstream checksum failure, and conservative commit after a fake upstream PUT attempt fails.
...
- Web UI coverage is typechecked, but there are no browser interaction tests for the new page in this branch.
- Route-level gateway budget tests cover the core remote-backed PUT/GET/DELETE, copy-object, multipart, deny-before-upstream, pre-upstream release, and post-upstream conservative commit paths. Additional provider-specific edge cases can still be expanded.
...
Live AWS CLI or MinIO `mc` smoke testing was not run in this environment because neither client binary is installed in the current shell. The smoke script added in `tools/smoke/s3_gateway_scoped_budget_smoke.sh` documents the intended manual/integration path and requires a configured Vaulthalla test instance; remote-cache steps additionally require upstream S3/R2 test environment variables.

## Next Hardening Steps

1. Run the smoke script against a real local gateway and at least one disposable upstream S3/R2 bucket.
2. Add web component tests for credential creation, secret reveal, scope editing, budget save/disable, and ledger rendering.
