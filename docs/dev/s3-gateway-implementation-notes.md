...
- Non-admin `vault_allowlist` creation/update now requires the principal to have real access to every named vault. Legacy shorthand scope rows are mirrored into gateway credential vault-role assignments, and request authorization still requires the principal's matching RBAC action.
