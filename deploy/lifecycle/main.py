...
def render_managed_nginx_config(projection: dict[str, dict[str, Any]], domain: str | None) -> str:
    ...
    for s3_location in ("= /api/s3", "/api/s3/"):
        out.extend([
            ...
            "        proxy_set_header X-Vaulthalla-S3-Base-Prefix /api/s3;",
            "    }",
            "",
        ])
...
def setup_nginx(args: argparse.Namespace) -> int:
    ...
    if args.certbot and not domain:
        raise LifecycleError(
            "--certbot requires --domain <domain> unless an existing managed nginx site already has a domain"
        )
    ...
    rendered = render_managed_nginx_config(projection, domain)
